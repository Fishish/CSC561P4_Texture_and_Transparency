...
var textureBuffers = []; // buffer for texture
...
function handleKeyDown(event) {
    ...
    switch (event.code) {
        ...
        case "Backspace": // reset model transforms to default
            ...
            break;
        case "KeyB":
            if (replaceModulate == true) {
                replaceModulate = false;
            } else {
                replaceModulate = true;
            }
            break;
    ...
    try {
        if (inputTriangles == String.null)
            ...
            for (var whichSet=0; whichSet<numTriangleSets; whichSet++) { // for each tri set
                ...
                inputTriangles[whichSet].glTexture = loadTexture(gl, "https://ncsucgclass.github.io/prog4/" + inputTriangles[whichSet].material.texture);
                ...
                for (whichSetVert=0; whichSetVert<numVerts; whichSetVert++) { // verts in set
                    ...
                    inputTriangles[whichSet].glUVs.push(inputTriangles[whichSet].uvs[whichSetVert][0],inputTriangles[whichSet].uvs[whichSetVert][1]);
...
function setupShaders() {
    ...
    var fShaderCode = `
        ...
        void main(void) {
            ...
            // gl_FragColor = texture2D(uSampler, vec2(1.0 - vTextureCoord.x, 1.0 - vTextureCoord.y));

            if(replaceModulate){
                gl_FragColor = texture2D(uSampler, vec2(1.0 - vTextureCoord.x, 1.0 - vTextureCoord.y)) * vec4(colorOut, uAlpha);
            }else{
                gl_FragColor = texture2D(uSampler, vec2(1.0 - vTextureCoord.x, 1.0 - vTextureCoord.y));
            }
        }
    ...
    try {
        ...
        if (!gl.getShaderParameter(fShader, gl.COMPILE_STATUS)) { // bad frag shader compile
            ...
            if (!gl.getProgramParameter(shaderProgram, gl.LINK_STATUS)) { // bad program link
                ...
                textureUalpha = gl.getUniformLocation(shaderProgram, "uAlpha");
                textureReplaceModulate = gl.getUniformLocation(shaderProgram, "replaceModulate");
...
function renderModels() {
    ...
    for (var whichTriSet=0; whichTriSet<numTriangleSets; whichTriSet++) {
        ...
        gl.uniform1f(textureReplaceModulate, replaceModulate);
        gl.uniform1f(textureUalpha, inputTriangles[whichTriSet].material.alpha);
